...
async function handleCheckAuth() {
  ...
  let userId = stored.current_user_id || null;
  let hasUser = !!userId;

  if (!hasUser) {
    try {
      const twidCookie = await chrome.cookies.get({ url: "https://x.com", name: "twid" });
      if (twidCookie?.value) {
        userId = twidCookie.value.replace("u%3D", "");
        hasUser = !!userId;
        if (hasUser) {
  ...
  return {
    hasUser,
    hasAuth: !!(stored.tw_auth_headers?.authorization),
